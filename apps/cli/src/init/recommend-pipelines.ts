...
async function promptForRecommendationAcceptance(): Promise<boolean> {
  const readline = createInterface({ input: process.stdin, output: process.stdout });
  try {
    const answer = (await readline.question("Create it now? [Y/n/skip] ")).trim().toLowerCase();
...
async function promptForAppAccessInstructions(): Promise<string | null> {
  ...
  try {
    for (;;) {
      const answer = (
        await readline.question("Application access instructions (type 'skip' to skip setup): ")
      ).trim();
...
async function createRecommendedSetupForProject(input: {
  ...
  if (!pipelineResponse.ok) {
    const errorBody = (await pipelineResponse.json().catch(() => null)) as
      | { title?: string }
      | null;
    throw new ConfigurationError(
...
export async function recommendPipelines(input: {
  ...
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    logger.info("Skipping recommended setup creation because this session is non-interactive.");
    return;
  }

  const accepted = await promptForRecommendationAcceptance();
  if (!accepted) {
    logger.info("Skipped recommended setup.");
    return;
  }

  const appAccessInstructions = await promptForAppAccessInstructions();
  if (!appAccessInstructions) {
    logger.info("Skipped recommended setup.");
    return;
  }

  const pipeline = await createRecommendedSetupForProject({ ...input, recommendation, appAccessInstructions });
  logger.info(
